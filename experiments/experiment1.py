    ...
    try:
        for key in key_list:
            if keyboard.is_pressed(key):
                print('key {} is pressed, target gesture is number {}'.format(key, key_list.index(key)+1))
                _list = [[name+1, key_list.index(key)+1, LR_idx[0], bool(name+1 == key_list.index(key)+1)]]
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            for i in range(len(mark_p_list)): #for문 한 번 도는게 한 손에 대한 것임
                ...
                try:
                    keyboard_check(HM.return_finger_state(experiment_mode=True), static_gesture_name, LR_idx)
                except:
                    pass
        ...
        if cv2.waitKey(5) & 0xFF == 27:
            ...
            experiment_df.to_csv('experiment1/experiment1_' + name + '.csv', encoding='utf-8-sig')
