...
def process_dynamic_gesture(shared_array_dynamic, dynamic_value):
    ...
    class Graph():
        ...
        def __init__(self):
            ...
            plt.rc('figure', titlesize=self.BIGGER_SIZE) # fontsize of the figure title self.figure, self.ax = plt.subplots(1, 2, figsize=(12, 6), facecolor='lightgray')
    ...
    while True:
        ...
        plt.subplot(111, xlim=(0, 50), ylim=(-5, 5))

        for i in range(63):
            point_num = i//3
            xyz = i%3
            if xyz == 0:
                plot_x += shared_array_dynamic[i] - mark_before[i]
            if xyz == 1:
                plot_y += shared_array_dynamic[i] - mark_before[i]
            if xyz == 2:
                plot_z += shared_array_dynamic[i] - mark_before[i]

        # anim = animation.FuncAnimation(fig, animate=plot_x, init_func=init, frames=200,
        #                                interval=50, blit=False)
        plot_diff = abs(plot_x) + abs(plot_y)  # + abs(plot_z)
        plt.scatter(t, plot_diff)
        #plt.scatter(t, plot_y, s='c')
